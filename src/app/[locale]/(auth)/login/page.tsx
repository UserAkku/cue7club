  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        let verifyUrl = `/${locale}/verify-otp?email=${encodeURIComponent(email)}`;
        if (role) verifyUrl += `&role=${role}`;
      ...
      <div className="flex w-full flex-col justify-center px-4 sm:px-12 lg:w-1/2 xl:px-24">
        <div className="mx-auto w-full max-w-md">
          ...
          <h1 className="font-heading text-3xl font-bold tracking-tight text-foreground">
            {role === "CUSTOMER" ? "Sign in or Create an account" : "Welcome back"}
          </h1>
          <p className="mt-2 text-muted-foreground">
            {role === "CUSTOMER" ? "Enter your email to get started." : "Sign in to your account to continue."}
          </p>
