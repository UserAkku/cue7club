  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="font-heading text-3xl font-bold tracking-tight">Professionals</h1>
          <p className="text-muted-foreground mt-2">Manage professional onboarding and statuses.</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs uppercase bg-white/5 text-muted-foreground border-b border-white/5">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">Service Category</th>
              ...
              {pros.map((pro) => (
                <tr key={pro.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    {pro.status === "PENDING" && (
                      <Button size="sm" className="bg-success hover:bg-success/90 text-white">Approve</Button>
                    )}
